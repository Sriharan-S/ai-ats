...
const roadmapGlowColors = ["blue", "purple", "green", "orange"] as const;
...
export default function LearnPage() {
  ...
  useEffect(() => {
    requestJson<RoadmapDef[]>("/api/roadmaps").then(setRoadmaps);
  }, []);
        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-6">
          {filtered.map((r, i) => (
            <Link key={r.slug} to={`/learn/${r.slug}`} className="block group">
              <GlowCard customSize className="h-full border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900" glowColor={roadmapGlowColors[i % roadmapGlowColors.length]}>
                <div className="mb-4">
