...
export default function AtsPage() {
  ...
  const handleAnalyze = async (e: FormEvent) => {
    ...
    try {
      ...
      formData.append("leetcode_username", leetcode);
      formData.append("codeforces_username", codeforces);

      const data = await requestJson<AtsAnalysisResponse>("/api/ats-analyze", {
        method: "POST",
        body: formData,
      });
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-center w-full">
                  <label htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full h-32 border-2 border-slate-300 dark:border-slate-700 border-dashed rounded-lg cursor-pointer bg-slate-50 dark:bg-slate-950 hover:bg-slate-100 dark:bg-slate-800">
                    ...
                    <input id="dropzone-file" type="file" className="hidden" accept=".pdf,.docx" onChange={handleFileChange} ref={filterInputRef} />
                  </label>
                </div>
              </div>
